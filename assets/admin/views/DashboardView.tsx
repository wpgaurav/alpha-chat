...
export function DashboardView() {
	...
	async function processOnce() {
		...
		try {
			...
			await Promise.all( [
				refreshQueue(),
				adminApi.getSettings().then( ( s ) => setStats( s.stats ) ),
			] );
		} catch {
			/* ignore */
		} finally {
						...
						<div className="alpha-chat-queue">
							<div className="alpha-chat-queue__header">
								...
								<Button
									...
									isBusy={ processing }
									disabled={
										queue.pending + queue.in_progress === 0
									}
								>
							...
							<div className="alpha-chat-queue__grid">
								<div>
									<div className="alpha-chat-queue__label">
										{ __( 'Pending', 'alpha-chat' ) }
									</div>
									<div className="alpha-chat-queue__value">
										{ queue.pending }
									</div>
								</div>
								<div>
								...
								<div>
									...
									<div className="alpha-chat-queue__value is-error">
										{ queue.failed }
									</div>
								</div>
							</div>
